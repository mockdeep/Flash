...
# A card in a reverse deck. It anchors a Back item (the prompt), so the front is
# that item's text (inherited) while the answer and its gloss-side metadata come
# from the paired Front item(s). Distractors are inherited: a reverse miss
# accretes a Front-side decoy onto the anchored Back item.
class ReverseTextCard < TextCard
  ...
  # The answering Front item - the first one paired to this prompt. A reverse
